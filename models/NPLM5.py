...
class NPLMModel(BaseModel):
    ...
    def _fusion(self, cdd_news_embedding, his_news_embedding):
        ...
        cdd_news_embedding = F.normalize(cdd_news_embedding.unsqueeze(dim=2), dim=-1)
        his_news_embedding = F.normalize(his_news_embedding.unsqueeze(dim=1), dim=-1).transpose(-2, -1)
        ...
        fusion_tensor = torch.matmul(cdd_news_embedding, his_news_embedding) / math.sqrt(cdd_news_embedding.shape[-1])
