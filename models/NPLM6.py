...
class NPLMModel(BaseModel):
    def __init__(self, hparams, vocab, user_num):
        ...
        self.psdQueryProject = nn.Linear(self.filter_num, 128)
        ...
        self.query = nn.Parameter(torch.randn((1, 256), requires_grad=True))
        self.query1 = nn.Parameter(torch.randn((1, 128), requires_grad=True))
    ...
    def _fusion(self, cdd_news_embedding, his_news_embedding):
        ...
        c = torch.matmul(fusion_tensor, his_news_embedding.transpose(-2, -1))
        return c
    ...
    def _his_combine1(self, pooling_vectors):
        """ pack all history interaction result into one single vector
        ...
        Returns:
            pooling_vector: tensor of [batch_size, cdd_size, kernel_num]
        """
        pooling_vector = self._scaled_dp_attention(self.query1, pooling_vectors, pooling_vectors).squeeze(dim=-2)
        return pooling_vector
    def forward(self, x):
        ...
        c = self._fusion(cdd_news_repr, his_news_repr)  # [torch.Size([10, 5, 50, 20, 256])]
        pooling_vector=c*mask   #torch.Size([30, 5, 50, 20, 256])

        # # #注意力方法
        # #part 1
        # pooling_vector = self._his_combine(c.transpose(-2, -3))  # torch.Size([10, 5, 20, 150])
        ...
        his_news_repr = self.attention_net_with_w(outputh, final_hidden_stateh).view(self.batch_size, self.his_size,                                                                                 128)  # [30,50,128]
        # print(his_news_repr.size())#[20,50,128]

        psd_news_repr = self._his_combine(pooling_vector)  # torch.Size([10, 5, 50, 256])
        news_psd = self.Tanh(self.psdQueryProject(psd_news_repr))
        news_reper =self._his_combine1(news_psd)
        cdd_news_repr=cdd_news_repr+news_reper
