...
class NPLMModel(BaseModel):
    def __init__(self, hparams, vocab, user_num):
        ...
        self.learningToRank2 = nn.Linear(int((int((self.his_size - 3)/3 + 1) - 3)/3 + 1) * 2 * 2 * 16,1)
    ...
    def _click_predictor1(self, fusion_tensors):
        """ calculate batch of click probability
        ...
        # score_t = torch.bmm(cdd_news_repr, user_repr.transpose(-2, -1)).squeeze(dim=-1)
        # scores = torch.bmm(cdd_news_repr, user_repr.transpose(-2, -1)).squeeze(dim=-1)
        fusion_tensors = self.learningToRank2(fusion_tensors).squeeze(dim=-1)

        if self.cdd_size > 1:
            score = self.softmax(fusion_tensors)
        else:
            score = torch.sigmoid(fusion_tensors)
        return score
    ...
    def forward(self, x):
        ...
        cdd_news_repr, cnews_embedding_pretrained,fcdd_news_embedding = self.encoder(cdd_news,user_index=user_index)  # torch.Size([30, 5, 20, 150]))

        his_news = x['clicked_title'].long().to(self.device)
        his_news_repr, hnews_embedding_pretrained,fhis_news_embedding = self.encoder(his_news,user_index=user_index)  # torch.Size([30, 50, 20, 150])
        ...
        # fusion_tensor = self.learningToRank2(fusion_tensor).squeeze(dim=-1)  # [10,5,128]

        cdd = self._cnn_resnet(cdd_news_repr)  # [150,20,300]
        his = self._cnn_resnet(his_news_repr)  # [1500,20,300]

        cdd = cnews_embedding_pretrained.transpose(-1, -2) + cdd
        his = hnews_embedding_pretrained.transpose(-1, -2) + his
        ...
        cdd_news_repr = self.attention_net_with_w(output, final_hidden_state).view(self.batch_size, self.cdd_size,
                                                                                   128)  # [20,5,128]
        # cdd_news_repr=cdd_news_repr*fusion_tensor
        # cdd_news_repr=cdd_news_repr+fusion_tensor
        ...
        fusion_tensors = self._fusion(fcdd_news_embedding, fhis_news_embedding)
        attent = self._click_predictor1(fusion_tensors)
