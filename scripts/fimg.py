...
from models.FIMG import FIMGModel
...
if __name__ == "__main__":
    hparams = {
        ...
        'epochs': 2,  # 跑十轮5
        'scale': 'demo',  # demo模型
        'mode': 'train',  # 喂进去的数据是训练数据 对应prepare()不同策略
    ...
    FIMGModel = FIMGModel(vocab=vocab, hparams=hparams, user_num=len(loaders[0].dataset.uid2index)).to(hparams['device'])

    if hparams['mode'] == 'dev':
        # evaluate_dev(npfkModel, hparams, loaders[0], load=True,)
        FIMGModel.evaluate(hparams, loaders[0], load=True)

    elif hparams['mode'] == 'train':
        # train(npfkModel, hparams, loaders)
         FIMGModel.fit(hparams, loaders)

    elif hparams['mode'] == 'test':
        # test(npfkModel, hparams, loaders[0])
         FIMGModel.test(hparams, loaders[0])
    elif hparams['mode'] == 'tune':
         FIMGModel.tune(hparams, loaders)
