...
from models.FIMN import FIMNModel
...
if __name__ == "__main__":
    ...
    FIMNModel = FIMNModel(vocab=vocab, hparams=hparams, user_num=len(loaders[0].dataset.uid2index)).to(hparams['device'])

    if hparams['mode'] == 'dev':
        # evaluate_dev(npfkModel, hparams, loaders[0], load=True,)
        FIMNModel.evaluate(hparams, loaders[0], load=True)

    elif hparams['mode'] == 'train':
        # train(npfkModel, hparams, loaders)
         FIMNModel.fit(hparams, loaders)

    elif hparams['mode'] == 'test':
        # test(npfkModel, hparams, loaders[0])
         FIMNModel.test(hparams, loaders[0])
    elif hparams['mode'] == 'tune':
         FIMNModel.tune(hparams, loaders)