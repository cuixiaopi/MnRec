...
def recall_score(labels,scores, k=10):
    device = torch.device('cuda:0')
    scores = np.asarray(scores)
    labels = np.asarray(labels)


    scores = torch.from_numpy(scores)  # sub_img为numpy类型
    labels = torch.from_numpy(labels)

    rank = (-scores).argsort(dim=-1)
    cut = rank[:k]
    hit = labels.gather(-1, cut)
    m=torch.min(torch.Tensor([k]).to(device), labels.sum(-1).float())
    return (hit.sum(-1).float() / m).item()
...
def cal_metric(labels, preds, metrics):
    ...
    for metric in metrics:
        ...
        或者，任意给定一个负样本，所有正样本的score中有多大比例是大于该负类样本的score
        """
        if metric == "auc":
            auc = np.mean(
                [
                    roc_auc_score(each_labels, each_preds,multi_class='ovo')
        ...
        #
        #
        #     size = cal_dims(labels)
        #     pad_seq(labels, size)
        #     labels = np.asarray(labels)
        #
        #     recal_score = np.mean(
        #         [
        #             recall_score(each_labels, each_preds)
        #             for each_labels, each_preds in zip(labels, preds)
        #         ]
        #     )
        #     res["recall"] = round(recal_score, 4)
            ...
            for k in ndcg_list:
                ...
                ndcg_temp = np.mean(
                    [
                        ndcg_score(each_labels, each_preds, k) #第一个真正的标签，第二个预测的分数
                        for each_labels, each_preds in zip(labels, preds)
                    ]
                )
            ...
            for k in ndcg_list:
                ...
                res["recall@{0}".format(k)] = round(ndcg_temp, 4)
...
def prepare(hparams, path='/NEWS/Data/MIND', shuffle=True, news=False, pin_memory=True, num_workers=4, impr=False):
    ...
    if impr:
        ...
        if "bert" not in hparams:
            embedding = GloVe(dim=300, cache='/NEWS/Data/MIND/vector_cache')
